...
const AppContent = ({ user, signOut }) => {  const [file, setFile] = useState(null);
  ...
  const onFileUpload = async () => {
    ...
    try {      const apiUrl = import.meta.env.VITE_CLOUDFRONT_CUSTOM_DOMAIN_URL;
      ...
      const uploadData = await uploadResponse.json();      if (!uploadResponse.ok) throw new Error(uploadData.message || 'Upload failed');
      ...
      const downloadData = await downloadResponse.json();      if (!downloadResponse.ok) throw new Error(downloadData.message || 'Could not get download link');
  ...
  const handleUpgrade = async () => {
    ...
    try {      const apiUrl = import.meta.env.VITE_CLOUDFRONT_CUSTOM_DOMAIN_URL;
      ...
      const data = await response.json();      if (!response.ok) throw new Error(data.message || 'Upgrade failed');
    ...
    <Flex direction="column" alignItems="center" justifyContent="center" minHeight="100vh" padding="2rem" background="var(--amplify-colors-background-secondary)">
      <Card variation="elevated" width="100%" maxWidth="500px">
        ...
        <Flex as="form" direction="column" marginTop="2rem" gap="1rem">
          <input type="file" onChange={onFileChange} />          <Button onClick={onFileUpload} variation="primary" isDisabled={!file || isUploading} isLoading={isUploading}>Upload File</Button>
        </Flex>
